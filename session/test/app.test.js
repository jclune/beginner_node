  ...
  describe('/login のテスト', function() {
    it('エラーメッセージは何もない', function(done) {
      browser.visit('/login').then(function() {
        browser.queryAll('div.error').should.be.empty;
        done();
      });
    });
    ...
    it('チェックがないとエラーメッセージ', function(done) {
      browser.visit('/login').then(function() {
        ...
        browser.queryAll('div.error').should.be.lengthOf(1);
        done();
      });
    });
    ...
    describe('管理者のテスト', function() {
      it('チェックだけだとメッセージが２つでて、チェックは残ってる', function(done) {
        browser.visit('/login').then(function() {
          ...
          browser.queryAll('div.error').should.be.lengthOf(2);
          browser.query('form input[name="userType"][value=admin]').checked.should.be.ok;
          done();
        });
      });
      ...
      it('名前だけだとエラーがでて、テキストボックスに値が残っている', function(done) {
        browser.visit('/login').then(function() {
          return browser
            .choose('admin')
            .fill('user', admin.name)
            .pressButton('form input[type="submit"]');
        }).then(function() {
          browser.queryAll('div.error').should.be.lengthOf(1);
          browser.query('form input[name="userType"][value=admin]').checked.should.be.ok;
          browser.field('user').value.should.equal(admin.name);
          done();
        });
      });
      ...
      it('パスワードだけだとエラーがでて、テキストボックスは空', function(done) {
        browser.visit('/login').then(function() {
          ...
          browser.queryAll('div.error').should.be.lengthOf(1);
          browser.query('form input[name="userType"][value=admin]').checked.should.be.ok;
          browser.field('password').value.should.be.empty;
          done();
        });
      });
